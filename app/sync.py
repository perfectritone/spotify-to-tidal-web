...
async def run_sync_streaming(
    ...
    # Playlists
    if sync_playlists:
        yield event({"type": "start", "task": "playlists", "label": "Playlists"})
        try:
            ...
            yield event({"type": "done", "task": "playlists", "result": result['playlists']})
        except Exception as e:
            result['playlists'] = {'error': str(e)}
            yield event({"type": "error", "task": "playlists", "error": str(e)})

    # Favorites
    if do_sync_favorites:
        yield event({"type": "start", "task": "favorites", "label": "Liked Songs"})
        try:
            await lib_sync_favorites(spotify, tidal_session, config)
            result['favorites'] = {'added': 0, 'not_found': []}
            yield event({"type": "done", "task": "favorites", "result": result['favorites']})
        except Exception as e:
            result['favorites'] = {'error': str(e)}
            yield event({"type": "error", "task": "favorites", "error": str(e)})

    # Albums
    if do_sync_albums:
        yield event({"type": "start", "task": "albums", "label": "Albums"})
        try:
            await lib_sync_albums(spotify, tidal_session, config)
            result['albums'] = {'added': 0, 'not_found': []}
            yield event({"type": "done", "task": "albums", "result": result['albums']})
        except Exception as e:
            result['albums'] = {'error': str(e)}
            yield event({"type": "error", "task": "albums", "error": str(e)})

    # Artists
    if do_sync_artists:
        yield event({"type": "start", "task": "artists", "label": "Artists"})
        try:
            await lib_sync_artists(spotify, tidal_session, config)
            result['artists'] = {'added': 0, 'not_found': []}
            yield event({"type": "done", "task": "artists", "result": result['artists']})
        except Exception as e:
            result['artists'] = {'error': str(e)}
            yield event({"type": "error", "task": "artists", "error": str(e)})
...
async def run_sync(
    ...
    async for event in run_sync_streaming(
        ...
        # Parse the event to extract the final result
        import json
